...
function loadAreas(geodata_root) {
    areas = [];
    for(var idx=0; idx<geodata_root.features.length;idx++) {
        areas.push(geodata_root.features[idx].properties);
    }
}

var loadCache = {};

async function lookupAreas(min_lon,max_lon,min_lat,max_lat) {
    var result = {};
...
async function updateDataLayers() {
    var bounds = getMapBoundaries();
    var area_geojsons = await lookupAreas(bounds.min_lon,bounds.max_lon,bounds.min_lat,bounds.max_lat);
    if (dataLayers) {
        for (var fn in dataLayers) {
            if (!(fn in area_geojsons)) {
                dataLayers[fn].remove();
                delete dataLayers[fn];
            }
        }
        for (var fn in area_geojsons) {
            if (!(fn in dataLayers)) {
                dataLayers[fn] = L.geoJson(area_geojsons[fn], {style: style, onEachFeature: onEachFeature}).addTo(mymap);
            }
        }
    }
...
async function locate(lon,lat) {
    var point1 = turf.point([lon,lat], { });//x,y
    showOnMap(lon,lat);

    var area_geojsons = await updateDataLayers();
...
async function boot() {
    ...
    var geojson_root = await fetch("data/output.geojson");
    var geodata_root = await geojson_root.json();
    loadAreas(geodata_root);

    times = geodata_root["properties"]["times"];

    await locate(lon,lat);
